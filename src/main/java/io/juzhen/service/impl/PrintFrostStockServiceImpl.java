...
import io.juzhen.vo.business.PrintFrostStockVO;
 ...
 */
@Service
public class PrintFrostStockServiceImpl extends AssemblePDFData {

    @Override
    protected <T> Object assembleCustinfoQueryDTO(T t) {
        PrintFrostStockVO printFrostStockVO = (PrintFrostStockVO) t;
        CustinfoQueryDTO custinfoQueryDTO = new CustinfoQueryDTO(10, 1);
        custinfoQueryDTO.setTaaccountid(printFrostStockVO.getAccount());
    ...
    protected Map getHeadInfo(Map map) {
        ...
        String className = PrintFrostStockVO.class.getSimpleName();
        headInfo.put("firstHeadInfo", "某某交易中心    托管公司（管理）");
        headInfo.put("secondHeadInfo", "【份额冻结】凭证");
        headInfo.put("orderId", ((PrintFrostStockVO) map.get(className)).getOrderId());
        return headInfo;
    }

    @Override
    public void createPdfTableData(Font fontChina12, PdfPTable table, Map resMap) {
        PrintFrostStockVO printFrostStockVO = (PrintFrostStockVO) resMap.get(PrintFrostStockVO.class.getSimpleName());
        /* 客户信息 */
        ...
        PdfUtil.addTableCell(table, printFrostStockVO.getFundName(), fontChina12, true, false, 2, 0);
		
		/* 产品代码 */
        PdfUtil.addTableCell(table, "产品代码", fontChina12, false, false, 0, 0);
        PdfUtil.addTableCell(table, printFrostStockVO.getFundCode(), fontChina12, true, false, 2, 0);
		
		/* 份额性质 */
        PdfUtil.addTableCell(table, "份额性质", fontChina12, false, false, 0, 0);
        PdfUtil.addTableCell(table, printFrostStockVO.getStockNatureName(), fontChina12, true, false, 2, 0);
		
		/* 冻结份额 */
        PdfUtil.addTableCell(table, "冻结份额", fontChina12, false, false, 0, 0);
        PdfUtil.addTableCell(table, printFrostStockVO.getFrostNum(), fontChina12, true, false, 2, 0);
		
		/* 冻结类别 */
        PdfUtil.addTableCell(table, "冻结类别", fontChina12, false, false, 0, 0);
        PdfUtil.addTableCell(table, printFrostStockVO.getFrostTypeName(), fontChina12, true, false, 2, 0);
		/* 这里为空 */
        ...
        PdfUtil.addTableCell(table, JuDataTypeUtils.getStringValue(printFrostStockVO.getRemark()), fontChina12, true, false, 5, 0);
    }
    ...
    public void createPdfEnd(Document document, BaseFont bfChinese, Font fontChina12, Paragraph blank1, Map resMap) throws DocumentException {
        ...
        // 操作柜员 与 业务章
        PdfUtil.addChunk(pE1, fontChina12, "操作柜员：", printFrostStockVO.getOperName(), 0);
